...
import { findFileById } from '../utils/fakeData';
...
const FileDetails = () => {
  ...
  const downloadFile = () => {
    if (!file) return;
    setDownloading(true);
    // FE độc lập: giả lập tải xuống bằng cách tạo blob từ text
    const blob = new Blob([`Demo content of ${file.name}`], { type: 'text/plain' });
    ...
    toast.success('Tải xuống (giả) thành công!');
  ...
  const updateFileName = async (e) => {
    ...
    setUpdating(true);
    setFile({ ...file, name: newFullFileName });
    toast.success('Đổi tên file (giả) thành công!');
    setIsRenaming(false);
    setUpdating(false);
  };
